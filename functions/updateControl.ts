/**
 * NW-UPGRADE-076D-PHASE3: Update control / ControlLibrary (write route).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const Entity = (svc.entities as Record<string, { update: (id: string, p: Record<string, unknown>) => Promise<unknown> }>).ControlLibrary;
    if (!Entity?.update) {
    ...
    console.error('updateControl error:', error);
