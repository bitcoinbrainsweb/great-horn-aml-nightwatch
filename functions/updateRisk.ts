/**
 * NW-UPGRADE-076D-PHASE3: Update engagement risk (write route).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const Entity = (svc.entities as Record<string, { update: (id: string, p: Record<string, unknown>) => Promise<unknown> }>).EngagementRisk;
    if (!Entity?.update) {
      return Response.json({ error: 'EngagementRisk entity not available' }, { status: 503 });
    }

    const updated = await Entity.update(id, payload);
    return Response.json({ risk: updated });
  } catch (error) {
