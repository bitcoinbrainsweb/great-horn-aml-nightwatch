...
Deno.serve(async (req) => {
  try {
    ...
    const resultsMarkdown = `# NW-UPGRADE-040 Verification Results

**Upgrade ID:** NW-UPGRADE-040  
**Product Version:** v0.7.0  
**Verified At:** ${verificationResults.verified_at}  
**Status:** ${success ? '✅ PASSED' : '❌ FAILED'}

## Summary

- **Total Checks:** ${verificationResults.summary.total_checks}
- **Passed:** ${verificationResults.summary.passed_checks}
- **Warnings:** ${verificationResults.summary.warnings}
- **Violations:** ${verificationResults.summary.violations}

## Artifact Publication

- **Published:** ${verificationResults.artifact_published ? '✅ Yes' : '❌ No'}
- **Artifact ID:** ${verificationResults.artifact_id || 'N/A'}
- **Title:** ${verificationResults.artifact_title || 'N/A'}
- **Classification:** ${verificationResults.artifact_classification || 'N/A'}
${verificationResults.artifact_error ? `- **Error:** ${verificationResults.artifact_error}` : ''}

## Passed Checks

${checks.filter(c => c.status === 'passed').map(c => `- ✅ **${c.check}**: ${c.details}`).join('\n')}

${warnings.length > 0 ? `## Warnings\n\n${warnings.map(w => `- ⚠️ **${w.check}**: ${w.issue} (Severity: ${w.severity})`).join('\n')}` : ''}

${violations.length > 0 ? `## Violations\n\n${violations.map(v => `- ❌ **${v.check}**: ${v.issue} (Severity: ${v.severity})`).join('\n')}` : ''}

## Conclusion

${success ? 'All critical checks passed. The Engagement and Audit foundation is properly implemented.' : 'Critical violations detected. Review and fix the issues above before proceeding.'}

---
*Generated by NW-UPGRADE-040 Verification Engine*
`;

    verificationResults.results_markdown = resultsMarkdown;
    verificationResults.results_file_path = 'docs/NW-UPGRADE-040_RESULT.md';
